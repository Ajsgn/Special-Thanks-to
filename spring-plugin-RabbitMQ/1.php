...
if ($_POST['type']) {
    ...
    switch ($type) {
        ...
        case 'setsd':
            ...
            break;
        case 'rolenum':
            $cmd = $flag2 . 'java -classpath .' . $flag . 'gsxdb.jar Clients ' . $port . ' ' . $uid . ' rolenum';
            exec($cmd, $out);
            $return = array(
                'errcode' => 0,
                'info' => '在线人数:' . $out[0],
        ...
        case 'gmcmd':
            ...
            $cmd = $flag2 . 'java -classpath .' . $flag . 'gsxdb.jar Clients ' . $port . ' ' . $uid . ' ' . $gmcmd;
            exec($cmd, $out);
            $return = array(
                'errcode' => 0,
                'info' => '执行成功',
            ...
            break;
        case 'updateshield':
            $cmd = $flag2 . 'java -classpath .' . $flag . 'gsxdb.jar Clients ' . $port . ' ' . $uid . ' updateshield';
            exec($cmd, $out);
            $return = array(
                'errcode' => 0,
                'info' => '更新成功',
            ...
            break;
        case 'updatenotclear':
            $cmd = $flag2 . 'java -classpath .' . $flag . 'gsxdb.jar Clients ' . $port . ' ' . $uid . ' updatenotclear';
            exec($cmd, $out);
            $return = array(
                'errcode' => 0,
                'info' => '更新成功',
        ...
        case 'makeequip':
            ...
            $cmd = $flag2 . "java -classpath ." . $flag . "gsxdb.jar Clients " . $port . " " . $uid . " addsequip#" . $equipid . "#" . $effectid . "#" . $skillid;
            exec($cmd, $out);
            $return = array(
                'errcode' => 0,
                'info' => $cmd,
